    ...
    try {
        ...
        if ([string]$variablesModule = $filesToCommit -match "Variables\.psm1") {
            ...
            if ($badNewVariables = $newVariable | ? { $_ -notmatch $optimalVariableNameRegex }) {
                ...
                _WarningAndExit "New variable$s`: $($badNewVariables -join ', ') doesn't start with '_', which is highly recommended to distinct them from variables defined in scripts or functions themselves."
            }
